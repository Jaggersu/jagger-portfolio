...
interface PartyPopperIconProps extends AnimatedIconProps {
  loop?: boolean;
}

const PartyPopperIcon = forwardRef<AnimatedIconHandle, PartyPopperIconProps>(
  (
    { size = 24, color = "currentColor", strokeWidth = 2, className = "", loop = false },
    ref,
  ) => {
    const [scope, animate] = useAnimate();

    const start = useCallback(async () => {
      // Reset values to restart animation cleanly
      animate(".popper, .confetti", { y: 0, x: 0, opacity: 1, scale: 1 }, { duration: 0 });
    ...
    useEffect(() => {
      ...
      const interval = setInterval(() => {
        start();
      }, 1000); // Trigger every 1 second as requested
