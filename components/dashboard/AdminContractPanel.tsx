...
const CONTRACT_CLAUSES = [
    {
        num: '01',
        title: '服務範疇',
        body: '乙方依客戶所選方案，提供對應之視覺設計、網頁建置或 SaaS 產品開發等服務。具體交付項目、規格與時程，以本線上系統生成之確認數據為準。超出本合約約定範圍之額外需求須另行報價。',
    },
    {
        num: '02',
        title: '修改次數與範圍變更',
        body: '本案交付包含最多 2 次微調修正。超出次數之修改，或因客戶需求變更導致工作範圍擴大者，乙方得另行報價。客戶如於確認稿後要求重大方向調整，視同新增需求處理。',
    },
    {
        num: '03',
        title: '費用、付款與執行時程',
        body: '本合約服務費用總計為 [[AMOUNT]]，執行時程為 [[TIMELINE]]（自客戶完整提供執行所需素材之次日起算）。本服務採 100% 線上全額預付制，客戶完成付款後，合約始生履約效力，乙方即安排時程執行。',
    },
    {
        num: '04',
        title: '智慧財產權歸屬',
        body: '客戶完成全額付款後，乙方將本專案之最終定稿著作財產權完整移轉予客戶。乙方保留：(a) 作品集展示與參展權；(b) 工作流程中所使用之通用框架、元件庫及可重用程式碼之所有權。未獲客戶採用之提案稿，著作權仍歸乙方所有。',
    },
    {
        num: '05',
        title: '客戶素材與侵權責任',
        body: '客戶提供之文字、圖片、商標及其他素材，應保證合法取得且不侵害任何第三方之智慧財產權；因客戶素材引發之法律責任由客戶自行承擔。乙方所使用之正版圖庫及字型授權，僅限本專案用途，客戶不得另作他用或轉授權。',
    },
    {
        num: '06',
        title: '合約終止與退款',
        body: '本案因屬客製化按件計酬服務，付款後若乙方已開始執行：(a) 客戶主動終止時，已支付款項不予退還，乙方應交付終止前已完成之階段性半成品；(b) 乙方主動終止時，應於 7 個工作天內依未完成比例退還對應款項。',
    },
    {
        num: '07',
        title: '保密條款',
        body: '雙方同意對合作過程中取得之商業機密、未公開素材、客戶資料予以嚴格保密；未經對方書面同意，不得向第三方揭露。保密義務於合約終止後繼續存續 3 年。',
    },
    {
        num: '08',
        title: '不可抗力',
        body: '因天災、政府法規變動、網路基礎設施故障、第三方服務中斷（如雲端平台、金流閘道）等不可抗力事件，導致乙方無法如期履約者，乙方得就受影響部分順延時程，雙方均不得以此為由要求違約賠償。',
    },
    {
        num: '09',
        title: '準據法與爭議解決',
        body: '本合約受中華民國法律管轄。雙方應先以協商方式解決爭議；協商不成時，同意以臺灣桃園地方法院為第一審管轄法院。',
    },
];
...
export default function AdminContractPanel() {
    ...
    // Group clients from the filtered contracts
    ...
    // If selected client is no longer in the list (e.g. filtered out):
    useEffect(() => {
        if (selectedClientId && !clientList.some(c => c.id === selectedClientId)) {
            setSelectedClientId(null);
            setSelected(null);
        }
    }, [clientList, selectedClientId]);

    // Contracts of the selected client
    const clientContracts = useMemo(() => {
        if (!selectedClientId) return [];
        return filteredContracts.filter(c => c.user_id === selectedClientId);
    }, [filteredContracts, selectedClientId]);
    ...
    const handlePrintContract = (targetContract: any) => {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <div class="meta">
                    <div class="meta-item"><strong>Client:</strong> ${targetContract.client_name || '—'}</div>
                    <div class="meta-item"><strong>Email:</strong> ${targetContract.client_email || '—'}</div>
                ...
                <div class="sig-section">
                    <div>
                        ...
                        <div style="margin-top: 6px; font-size: 11px; color: #555;">
                            代表人 (Representative): ${targetContract.client_name || '—'}<br/>
    ...
    return (
        <div className="flex h-full w-full overflow-hidden divide-x divide-zinc-900/60">
            ...
            <div className="w-[240px] shrink-0 flex flex-col bg-[#000000]">
                <div className="px-4 py-3 border-b border-zinc-900 space-y-2.5 shrink-0">
                    <div className="flex items-center justify-between">
                        <div className="text-xs text-zinc-600 tracking-widest">// CLIENTS</div>
                        <div className="text-[11px] text-zinc-500 font-mono">{clientList.length}</div>
                    </div>
                    <input
                        type="text"
                        value={search}
                        onChange={e => setSearch(e.target.value)}
                        placeholder="搜尋客戶 / 合約 / 專案…"
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-zinc-900/30">
                    {clientList.length === 0 ? (
                        <div className="px-4 py-12 text-center text-zinc-700 text-xs">// NO CLIENTS</div>
            ...
            {/* 2nd Column: Selected Client's Contracts List */}
            <div className="w-[260px] shrink-0 flex flex-col bg-[#080809]">
                <div className="px-4 py-[17px] border-b border-zinc-900 shrink-0">
                    <div className="text-xs text-zinc-600 tracking-widest">// CONTRACT LIST</div>
                </div>
                <div className="flex-1 overflow-y-auto divide-y divide-zinc-900/30">
                    {!selectedClientId ? (
                        <div className="px-4 py-12 text-center text-zinc-700 text-xs italic">// 請先選擇客戶</div>
                    ) : clientContracts.length === 0 ? (
                        <div className="px-4 py-12 text-center text-zinc-700 text-xs">// 查無合約</div>
                        ...
                        return (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 transition-colors ${isSelected ? 'bg-zinc-900/80' : 'hover:bg-zinc-900/30'}`}
                            >
            ...
            {/* 3rd Column: Contract details */}
            <div className="flex-1 overflow-y-auto px-6 py-5 bg-[#000000]">
                {!selected ? (
                    <div className="flex flex-col items-center justify-center h-full gap-2 text-zinc-700">
                        ...
                        <span className="text-xs">從左欄選擇客戶與合約來查看內容</span>
                    </div>
                ) : (
                    <div className="space-y-5 max-w-3xl">
                        ...
                        <div className="flex items-start justify-between border-b border-zinc-900 pb-4">
                            <div>
                                <div className="text-[10px] text-zinc-600 tracking-widest mb-1 font-mono">// CONTRACT DETAIL</div>
                                <h2 className="text-sm font-bold text-white font-mono">{selected.metadata?.plan || selected.plan_type} 服務協議</h2>
                                <div className="text-xs text-zinc-500 mt-1">{selected.client_name} · {selected.client_email}</div>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                                <span className={`text-[10px] px-2 py-0.5 rounded border ${selected.status === 'SIGNED' ? 'text-emerald-400 border-emerald-900 bg-emerald-950/30' : 'text-zinc-500 border-zinc-850'}`}>
                                    {selected.status === 'SIGNED' ? '已簽署' : '待付款'}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs">
                            <div>
                                <span className="text-[10px] text-zinc-600 block mb-0.5">PLAN</span>
                                <span className="text-zinc-200 font-mono">{selected.metadata?.plan || selected.plan_type || '—'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] text-zinc-600 block mb-0.5">AMOUNT</span>
                                <span className="text-[#FF5500] font-mono">NT$ {Number(selected.metadata?.amount || 0).toLocaleString()}</span>
                            </div>
                            <div>
                                <span className="text-[10px] text-zinc-600 block mb-0.5">TIMELINE</span>
                                <span className="text-zinc-200">{selected.metadata?.timeline || '按月續約'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] text-zinc-600 block mb-0.5">STATUS</span>
                                <span className={selected.status === 'SIGNED' ? 'text-emerald-400' : 'text-yellow-500'}>
                                    {selected.status === 'SIGNED' ? '✓ 已簽署' : '待付款'}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-3 pt-2">
                            <button
                                onClick={() => handlePrintContract(selected)}
                                className="py-2.5 border border-zinc-800 text-zinc-350 hover:text-white rounded-lg text-xs font-mono font-bold hover:border-zinc-700 transition-colors"
                            >
                                🖨 列印合約
                            </button>
                        </div>
